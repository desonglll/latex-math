...
\begin{enumerate}
    \item 无界函数 $f(x)$在 $[a,b]$内 一定不存在定积分
...
\begin{enumerate}
    ...
    \item \underline{$f(x)$ 可积，即函数连续或者存在震荡间断点}
    \item \underline{$f(x)$在 $[a,b]$ 有界，至多有有限个间断点}
\end{enumerate}
...
\begin{enumerate}
    \item $\displaystyle\hspace{.2em}\bar{x}=\frac{\int_{a}^{b}x\rho(x)ds}{\int_{a}^{b}\rho(x)ds}$
    \item $\displaystyle\hspace{.2em}\bar{y}=\frac{\int_{a}^{b}y\rho(x)ds}{\int_{a}^{b}\rho(x)ds}$
\end{enumerate}

\subsection{形心公式}

\begin{enumerate}
    \item $\displaystyle\hspace{.2em}\bar{x}=\frac{\iint xdS}{S}$
    \item $\displaystyle\hspace{.2em}\bar{y}=\frac{\iint ydS}{S}$
\end{enumerate}
...
与$z$轴的夹角为$\varphi $（范围是$0\le\varphi\le\pi$），与$x$轴的夹角为$\theta$
...
\subsection{球面坐标下的三重积分}

$\displaystyle\hspace{.2em} \iiint_\Omega f(x,y,z)dv=\iiiint_\Omega f(r\sin \varphi\cos\theta,r\sin\varphi\sin\theta,r\cos\varphi)r^2 \sin\varphi drd\varphi d\theta$

记住 \underline{$\displaystyle\hspace{.2em} \iiint_{\Omega: x^2+y^2+z^2\le R} (x^2+y^2+z^2) dv=\frac{4\pi R^5}{5}$}
